...
export interface GenConfigOptions {
  /** Resolved namespace (package name or 'qa-studio'). */
  namespace:    string;
...
export function genConfigText(opts: GenConfigOptions): GenConfigResult {
  ...
  const typeImport = isTypeScript
    ...
    : `// @ts-check\n/** @type {import('qa-studio').QaConfig} */\n`;
  ...
  const header = [
    `/**`,
    ` * qa-studio config — generated by \`qa-studio init\``,
    ` *`,
    ` * Fill every TODO field before mounting <QaStudio config={config} />.`,
    ` * Re-run \`qa-studio init --force\` only if you want to regenerate from scratch`,
    ` * (your edits WILL be overwritten with --force).`,
    ` *`,
    ` * Schema reference: https://github.com/qa-studio/qa-studio#qaconfig`,
    hintsComment,
    ` */`,
  ]
    .filter(l => l !== '')
